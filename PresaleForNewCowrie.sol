...
contract PreSale is ReentrancyGuard, Context, Ownable {
    ...
    function addPhase(uint8 phaseNum,uint256 priceN, uint256 priceD, uint256 endDate, uint256 balance) public onlyOwner returns (bool) {
        require (phaseNum <= maxPhase, "Max phase number error.");        
    ...
    function getPhase(uint8 _phaseNum) public view returns (        
        ...
        uint256 depositAmount,
        uint256 remainAmount) {
        require (_phaseNum > 0 && _phaseNum <= maxPhase, "Pre-Sale: Phase number must be small than 9.");
    ...
    function remainIcoAmount()  public view returns(uint256) {
        uint256 amt = 0;
        for (uint8 i = 1; i <= maxPhase; i++) {
            amt = amt.add(_phaseList[i].remainAmount);
        }
        return amt;
    }    
    //Start Pre-Sale
    ...
    function BuyTokens() public nonReentrant icoActive payable {
        ...
        uint256 refundEthAmount  = tokenAmount == buyTokenAmount ? 0 : _getETHAmount(tokenAmount.sub(buyTokenAmount));
        ...
        //_deliverTokens(_msgSender(), buyTokenAmount);
    ...
    function getDeliverAmount (address buyer) public view returns(uint256)  {
        ...
        amount = _claimStatus[buyer].buyAmount;
        return amount;
    }
    
    function Deliver() public icoNotActive(){
        require (address(_token)!=address(0), 'Token is not set.');
        require (claimReady, 'Claiming is not ready.');
